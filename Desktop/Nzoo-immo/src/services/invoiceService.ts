...
const generateReservationInvoiceHTML = (data: ReservationData): string => {
  ...
  const dates = validateAndFormatDates(data.startDate, data.endDate, data.created_at);
  ...
  return `
    ...
    <body>
      <div class="invoice-container">
        <div class="header">
          <div class="logo">
            <img src="/logo-nzoo-immo-header.svg" alt="N'zoo Immo Logo" onerror="this.style.display='none'">
          </div>
          <h1>FACTURE</h1>
        ...
        <div class="invoice-details">
          ...
          <div class="details-grid">
            ...
            <div class="detail-item">
              ...
              <div class="detail-value">${dates.dueDate}</div>
            </div>
            <div class="detail-item">
...
const generateInvoiceHTML = (data: InvoiceData): string => {
  ...
  // Valider et formater toutes les dates
  const dates = validateAndFormatDates(reservation.start_date, reservation.end_date, reservation.created_at);
  ...
  return `
    ...
    <body>
          ...
          <div class="header">
            <div class="logo">
              <img src="${companyInfo.logo}" alt="N'zoo Immo Logo" onerror="this.style.display='none'">
            </div>
            <h1>FACTURE</h1>
            <p>N'zoo Immo - Espaces de Travail</p>
          </div>
...
export const generateAndDownloadInvoice = async (reservation: Reservation): Promise<void> => {
  try {
    ...
    const companyInfo = {
      ...
      phone: "+243822201758",
      email: "contact@nzoo-immo.com",
      logo: "/logo-nzoo-immo-header.svg" // Chemin vers le logo SVG
    };
...
export const generatePDFInvoice = async (reservation: Reservation): Promise<void> => {
  try {
    ...
    doc.text(`Échéance: ${dates.dueDate}`, 20, 130);
    doc.text(`Statut: ${reservation.status}`, 20, 135);
