...
export const getSpaceInfo = (spaceType: string, language: 'fr' | 'en'): SpaceInfo | null => {
  const defaultData = language === 'fr' ? spacesDataFr : spacesDataEn;
  const mergedData = SpaceContentService.mergeWithDefault(defaultData, language);
  return mergedData[spaceType] || null;
};

export const getAllSpaces = (language: 'fr' | 'en'): SpacesData => {
  const defaultData = language === 'fr' ? spacesDataFr : spacesDataEn;
  return SpaceContentService.mergeWithDefault(defaultData, language);
};

// Fonction pour obtenir les données par défaut (sans modifications)
export const getDefaultSpaces = (language: 'fr' | 'en'): SpacesData => {
