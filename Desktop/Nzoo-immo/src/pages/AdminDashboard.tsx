...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ language }) => {
  ...
  const handleDeleteClient = async (clientEmail: string) => {
    console.log('🗑️ Tentative de suppression du client:', clientEmail);
  ...
  useEffect(() => {
    ...
    // Si l'utilisateur essaie d'accéder à l'onglet utilisateurs sans être admin
    if (activeTab === 'users' && !isAdmin()) {
      console.log('🚫 Accès refusé à l\'onglet utilisateurs - Redirection vers overview');
      setActiveTab('overview');
    }
    
    // Si l'utilisateur essaie d'accéder aux onglets clients ou statistiques sans être admin
    if ((activeTab === 'clients' || activeTab === 'statistics') && !isAdmin()) {
      console.log('🚫 Accès refusé aux onglets clients/statistiques - Redirection vers overview');
      setActiveTab('overview');
    }
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center">
                    <div className="flex items-center justify-center space-x-2">
                      <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-nzoo-dark"></div>
  ...
  const tabs = [
    ...
    ...(userProfile?.role === 'clients' ? [
      ...
      // Pour admin et user, tous les onglets sauf gestion des réservations
      ...
      // Onglets clients et statistiques seulement visibles pour les admins
      ...(isAdmin() ? [{ id: 'clients', label: t.tabs.clients, icon: Users }] : []),
      ...(isAdmin() ? [{ id: 'statistics', label: t.tabs.statistics, icon: TrendingUp }] : []),
      // Onglet utilisateurs seulement visible pour les admins
      ...(isAdmin() ? [{ id: 'users', label: t.tabs.users, icon: Users }] : []),
      // Onglet profil pour tous les utilisateurs
      { id: 'profile', label: 'Mon Profil', icon: User }
    ])
  ];
