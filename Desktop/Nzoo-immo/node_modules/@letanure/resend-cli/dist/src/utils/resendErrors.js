 ...
 */
export function formatResendError(error, action, requestData) {
    if (error && typeof error === 'object' && 'name' in error && 'message' in error) {
        const resendError = error;
        const errorInfo = RESEND_ERROR_CODES[resendError.name] || RESEND_ERROR_CODES.unknown_error;
    ...
    const errorInfo = RESEND_ERROR_CODES[errorType] || RESEND_ERROR_CODES.unknown_error;
    // Use error message if available, or custom
    const message = errorObj?.message || errorInfo.message;
    const httpStatus = errorObj?.statusCode || errorObj?.status || errorInfo.status || 'Unknown';
    let result = `Failed to ${action}\n\n${message}\n\nSuggestion: ${errorInfo.suggestion}`;
...
//# sourceMappingURL=resendErrors.js.map