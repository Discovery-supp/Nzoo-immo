...
async function handleSendCommand(options, command) {
    ...
    try {
        const apiKey = getResendApiKey(allOptions.apiKey);
        // Extract output format and validate email data
        const outputFormat = allOptions.output || 'text';
        const emailData = validateOptions(allOptions, CreateEmailOptionsSchema, outputFormat, fields, command);
        // Check if dry-run mode is enabled
        const isDryRun = Boolean(allOptions.dryRun);
        // Use generic displayResults function
        const result = isDryRun ? undefined : await sendEmail(emailData, apiKey);
        displayResults({
            data: emailData,
            result,
            fields,
