...
export const Form = ({ onExit }) => {
    ...
    if (isSubmitting) {
        return (_jsx(Layout, { headerText: `${config.baseTitle} - Emails - Cancel`, showNavigationInstructions: false, navigationContext: "none", children: _jsx(Spinner, { label: "Cancelling email..." }) }));
    }
    if (successData) {
        return (_jsx(SuccessScreen, { data: successData, successMessage: "Email Cancelled Successfully", headerText: `${config.baseTitle} - Emails - Cancel`, isDryRun: isDryRunSuccess, onExit: () => {
                setSuccessData(null);
                setIsDryRunSuccess(false);
                onExit();
            } }));
    ...
    if (error) {
        return (_jsx(ErrorScreen, { title: error.title, message: error.message, suggestion: error.suggestion, headerText: `${config.baseTitle} - Emails - Cancel`, onExit: () => {
                setError(null);
                onExit();
    ...
    return (_jsx(Layout, { headerText: `${config.baseTitle} - Emails - Cancel`, showNavigationInstructions: true, navigationContext: "form-single", children: _jsx(SimpleForm, { fields: fields, onSubmit: handleSubmit, onCancel: onExit, validateWith: CancelEmailOptionsSchema }) }));
};
