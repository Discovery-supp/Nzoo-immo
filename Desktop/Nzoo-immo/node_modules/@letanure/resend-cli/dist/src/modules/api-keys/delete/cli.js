...
import { deleteApiKeySchema } from './schema.js';
async function handleDeleteCommand(options, command) {
    ...
    try {
        ...
        const validatedData = validateOptions(allOptions, deleteApiKeySchema, outputFormat, fields, command);
...
export function registerDeleteApiKeyCommand(apiKeysCommand) {
    const deleteCommand = createDeleteApiKeyCommand();
    apiKeysCommand.addCommand(deleteCommand);
}
function createDeleteApiKeyCommand() {
    const deleteCommand = new Command('delete')
        .description('Delete an existing API key from Resend')
        .action((options, command) => handleDeleteCommand(options, command));
