...
export const Form = ({ onExit }) => {
    ...
    if (isSubmitting) {
        return (_jsx(Layout, { headerText: `${config.baseTitle} - Domains - Create`, showNavigationInstructions: false, navigationContext: "none", children: _jsx(Spinner, { label: "Creating domain..." }) }));
    }
    if (successData) {
        return (_jsx(SuccessScreen, { data: successData, successMessage: "Domain Created Successfully", headerText: `${config.baseTitle} - Domains - Create`, isDryRun: isDryRunSuccess, onExit: () => {
                setSuccessData(null);
                setIsDryRunSuccess(false);
                onExit();
            } }));
    ...
    if (error) {
        return (_jsx(ErrorScreen, { title: error.title, message: error.message, suggestion: error.suggestion, headerText: `${config.baseTitle} - Domains - Create`, onExit: () => {
                setError(null);
                onExit();
    ...
    return (_jsx(Layout, { headerText: `${config.baseTitle} - Domains - Create`, showNavigationInstructions: true, navigationContext: "form", children: _jsx(Box, { flexDirection: "column", children: _jsx(SimpleForm, { fields: fields, onSubmit: handleSubmit, onCancel: onExit, validateWith: createDomainSchema }) }) }));
};
