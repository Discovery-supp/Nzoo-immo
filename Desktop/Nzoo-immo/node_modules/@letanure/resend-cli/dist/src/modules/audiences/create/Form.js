...
export const Form = ({ onExit }) => {
    ...
    if (isSubmitting) {
        return (_jsx(Layout, { headerText: `${config.baseTitle} - Audiences - Create`, showNavigationInstructions: false, navigationContext: "none", children: _jsx(Spinner, { label: "Creating audience..." }) }));
    }
    if (successData) {
        return (_jsx(SuccessScreen, { data: successData, successMessage: "Audience Created Successfully", headerText: `${config.baseTitle} - Audiences - Create`, isDryRun: isDryRunSuccess, onExit: () => {
                setSuccessData(null);
                setIsDryRunSuccess(false);
                onExit();
            } }));
    ...
    if (error) {
        return (_jsx(ErrorScreen, { title: error.title, message: error.message, suggestion: error.suggestion, headerText: `${config.baseTitle} - Audiences - Create`, onExit: () => {
                setError(null);
                onExit();
    ...
    return (_jsx(Layout, { headerText: `${config.baseTitle} - Audiences - Create`, showNavigationInstructions: true, navigationContext: "form-single", children: _jsx(SimpleForm, { fields: fields, onSubmit: handleSubmit, onCancel: onExit, validateWith: CreateAudienceOptionsSchema }) }));
};
