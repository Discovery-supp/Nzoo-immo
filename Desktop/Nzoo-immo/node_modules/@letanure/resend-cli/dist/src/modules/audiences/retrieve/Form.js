...
export const Form = ({ onExit }) => {
    ...
    if (isSubmitting) {
        return (_jsx(Layout, { headerText: `${config.baseTitle} - Audiences - Retrieve`, showNavigationInstructions: false, navigationContext: "none", children: _jsx(Spinner, { label: "Retrieving audience..." }) }));
    }
    if (successData) {
        return (_jsx(SuccessScreen, { data: successData, successMessage: "Audience Retrieved Successfully", headerText: `${config.baseTitle} - Audiences - Retrieve`, isDryRun: isDryRunSuccess, onExit: () => {
                setSuccessData(null);
                setIsDryRunSuccess(false);
                onExit();
            } }));
    ...
    if (error) {
        return (_jsx(ErrorScreen, { title: error.title, message: error.message, suggestion: error.suggestion, headerText: `${config.baseTitle} - Audiences - Retrieve`, onExit: () => {
                setError(null);
                onExit();
    ...
    return (_jsx(Layout, { headerText: `${config.baseTitle} - Audiences - Retrieve`, showNavigationInstructions: true, navigationContext: "form-single", children: _jsx(SimpleForm, { fields: formFields, onSubmit: handleSubmit, onCancel: onExit, validateWith: RetrieveAudienceOptionsSchema, initialData: initialFormData }) }));
};
