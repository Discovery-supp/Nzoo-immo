...
describe('Menu Component', () => {
    ...
    it('renders all menu items', () => {
        const { lastFrame } = renderWithProviders(_jsx(Menu, { menuItems: mockMenuItems, onSelect: mockOnSelect, onExit: mockOnExit }));
    ...
    it('highlights initial selected item when provided', () => {
        const { lastFrame } = renderWithProviders(_jsx(Menu, { menuItems: mockMenuItems, onSelect: mockOnSelect, onExit: mockOnExit, initialSelectedKey: "domains" }));
        expect(lastFrame()).toContain('▶');
    });
    it('navigates down with down arrow', async () => {
        const { stdin, lastFrame } = renderWithProviders(_jsx(Menu, { menuItems: mockMenuItems, onSelect: mockOnSelect, onExit: mockOnExit }));
        await stdin.write('\u001B[B'); // Down arrow
        const output = lastFrame();
        expect(output).toContain('▶');
    });
    it('navigates up with up arrow', async () => {
        const { stdin, lastFrame } = renderWithProviders(_jsx(Menu, { menuItems: mockMenuItems, onSelect: mockOnSelect, onExit: mockOnExit, initialSelectedKey: "domains" }));
        await stdin.write('\u001B[A'); // Up arrow
        const output = lastFrame();
        expect(output).toContain('▶');
    });
    it('calls onSelect when Enter is pressed', async () => {
        const { stdin } = renderWithProviders(_jsx(Menu, { menuItems: mockMenuItems, onSelect: mockOnSelect, onExit: mockOnExit }));
    ...
    it('handles empty menu items gracefully', () => {
        const { lastFrame } = renderWithProviders(_jsx(Menu, { menuItems: [], onSelect: mockOnSelect, onExit: mockOnExit }));
    ...
    it('wraps around when navigating down from last item', async () => {
        const { stdin, lastFrame } = renderWithProviders(_jsx(Menu, { menuItems: mockMenuItems, onSelect: mockOnSelect, onExit: mockOnExit, initialSelectedKey: "api-keys" }));
    ...
    it('circular navigation: down from last goes to first', async () => {
        const { stdin } = renderWithProviders(_jsx(Menu, { menuItems: mockMenuItems, onSelect: mockOnSelect, onExit: mockOnExit, initialSelectedKey: "api-keys" }));
