...
const ReservationManagement: React.FC<ReservationManagementProps> = ({ language }) => {
  ...
  console.log('ðŸ‘¤ ReservationManagement - Données utilisateur:', { 
  ...
  // Log pour déboguer
  console.log('ðŸ” ReservationManagement - Ã‰tat:', {
    user: user ? { email: user.email, role: user.role } : null,
  ...
  const translations = {
    fr: {
      ...
      adminPermissionsNote: 'En tant qu\'administrateur, vous avez accès Ã  toutes les fonctionnalités',
      saveSuccess: 'Réservation mise Ã  jour avec succès',
      saveError: 'Erreur lors de la sauvegarde',
      statusUpdateSuccess: 'Statut mis Ã  jour avec succès',
      statusUpdateError: 'Erreur lors du changement de statut',
      delete: 'Supprimer',
      deleteConfirm: 'ÃŠtes-vous sûr de vouloir supprimer cette réservation ?',
      deleteSuccess: 'Réservation supprimée avec succès',
      deleteError: 'Erreur lors de la suppression',
      sendEmail: 'Envoyer un email',
      sendManualEmail: 'Envoyer email manuellement',
      emailSuccess: 'Email envoyé avec succès',
      emailError: 'Erreur lors de l\'envoi de l\'email',
      manualEmailSuccess: 'Email manuel envoyé avec succès',
      manualEmailError: 'Erreur lors de l\'envoi de l\'email manuel',
      emailModalTitle: 'Envoyer un email personnalisé',
      emailModalSubject: 'Sujet de l\'email',
      emailModalMessage: 'Message personnalisé',
      emailModalSubjectPlaceholder: 'Ex: Confirmation de réservation',
      emailModalMessagePlaceholder: 'Écrivez votre message personnalisé ici...',
      emailModalSend: 'Envoyer l\'email',
      emailModalCancel: 'Annuler',
      emailModalLoading: 'Envoi en cours...',
      ...
      autoEmailSuccess: 'Email de confirmation envoyé automatiquement au client',
      autoEmailWarning: 'Statut mis à jour mais erreur lors de l\'envoi de l\'email de confirmation',
    ...
    en: {
      ...
      sendManualEmail: 'Send manual email',
      emailSuccess: 'Email sent successfully',
      emailError: 'Error while sending email',
      manualEmailSuccess: 'Manual email sent successfully',
      manualEmailError: 'Error while sending manual email',
      emailModalTitle: 'Send custom email',
      emailModalSubject: 'Email subject',
      emailModalMessage: 'Custom message',
      emailModalSubjectPlaceholder: 'Ex: Reservation confirmation',
      emailModalMessagePlaceholder: 'Write your custom message here...',
      emailModalSend: 'Send email',
      emailModalCancel: 'Cancel',
      emailModalLoading: 'Sending...',
      ...
      autoEmailSuccess: 'Confirmation email sent automatically to client',
      autoEmailWarning: 'Status updated but error sending confirmation email',
  ...
  // Fonction pour vérifier et mettre Ã  jour automatiquement les statuts selon les règles documentées
  const checkAndUpdateReservationStatuses = async () => {
    try {
      ...
      if (result.updatedCount > 0) {
        const message = `${result.updatedCount} réservation(s) mises Ã  jour automatiquement`;
        showNotification('info', message);
      }
      
      if (result.errors.length > 0) {
        console.error('Erreurs lors de la mise Ã  jour automatique:', result.errors);
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (user?.role === 'admin') {
        await updateReservation(editingReservation, editFormData as Reservation);
      } else if (editFormData.status) {
        // Utilisateur non-admin: ne peut modifier que le statut
  ...
  const handleStatusChange = async (reservationId: string, newStatus: Reservation['status']) => {
    try {
      ...
      // Envoyer automatiquement un email de confirmation si le statut passe à "confirmed"
  ...
  // Fonction pour ouvrir le modal d'email
  const handleOpenEmailModal = (reservation: Reservation) => {
    setEmailSubject(`Mise à jour de votre réservation - ${reservation.full_name}`);
    setEmailMessage(`Bonjour ${reservation.full_name},\n\nNous vous contactons concernant votre réservation.\n\nCordialement,\nL'équipe N'zoo Immo`);
    setShowEmailModal(reservation.id);
  };

  // Fonction pour envoyer un email personnalisé
  const handleSendCustomEmail = async (reservation: Reservation) => {
  ...
  // Fonction pour fermer le modal d'email
  ...
  const handleDownloadInvoice = async (reservation: Reservation) => {
    ...
    if (reservation.status !== 'confirmed') {
      showNotification('error', 'Seules les réservations confirmées peuvent générer une facture');
      return;
    }
    ...
    try {
      ...
      showNotification('success', 'Facture générée et téléchargée avec succès');
    } catch (error) {
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            title="Vérifier et mettre Ã  jour automatiquement les statuts des réservations"
          >
        ...
        {filteredReservations.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            ...
            <p className="text-lg font-medium">
              {user?.role === 'clients' ? 'Aucune réservation trouvée' : t.noReservations}
            </p>
            {user?.role === 'clients' ? (
              <div className="space-y-4 mt-4">
                <p className="text-sm text-gray-600">
                  Vous n'avez pas encore effectué de réservation avec votre compte.
                </p>
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 max-w-md mx-auto">
                  <h4 className="font-medium text-blue-900 mb-2">💡 Que faire maintenant ?</h4>
                  <ul className="text-sm text-blue-800 space-y-1">
                    <li>• Explorez nos espaces disponibles</li>
                    <li>• Créez votre première réservation</li>
                  ...
                  <tr key={reservation.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        ...
                        <button 
                          onClick={() => handleOpenEmailModal(reservation)}
                          className="text-green-600 hover:text-green-900 p-2 rounded-lg hover:bg-green-50 transition-colors"
                          title={t.sendManualEmail}
                        >
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <h4 className="text-sm font-medium text-gray-700 mb-2">Informations de la réservation :</h4>
                <div className="text-sm text-gray-600 space-y-1">
                  <p><strong>Client :</strong> {reservations.find(r => r.id === showEmailModal)?.full_name}</p>
                  <p><strong>Email :</strong> {reservations.find(r => r.id === showEmailModal)?.email}</p>
                  <p><strong>Espace :</strong> {reservations.find(r => r.id === showEmailModal)?.space_type}</p>
                  <p><strong>Période :</strong> {reservations.find(r => r.id === showEmailModal)?.start_date} - {reservations.find(r => r.id === showEmailModal)?.end_date}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end space-x-3 mt-6">
              ...
              <button
                onClick={() => handleSendCustomEmail(reservations.find(r => r.id === showEmailModal)!)}
                disabled={sendingEmail === showEmailModal || !emailSubject.trim() || !emailMessage.trim()}
              ...
              >
                {sendingEmail === showEmailModal ? (
                  <>
                    ...
                    <span>{t.emailModalLoading}</span>
                  </>
                ) : (
                  <>
                    <Send className="w-4 h-4" />
                    <span>{t.emailModalSend}</span>
                  </>
                )}
              </button>
            </div>
