...
const UserManagement: React.FC<UserManagementProps> = ({ language }) => {
  ...
  // Configurer la langue pour les messages
  React.useEffect(() => {
  ...
  const [formData, setFormData] = useState({
    ...
    password: '',
    confirmPassword: '',
    role: 'user' as 'admin' | 'user' | 'clients',
    full_name: '',
    is_active: true
  });
  ...
  const translations = {
    fr: {
      ...
      messages: {
        confirmDelete: 'ÃŠtes-vous sûr de vouloir supprimer cet utilisateur ?',
        passwordMismatch: 'Les mots de passe ne correspondent pas',
        usernameRequired: 'Le nom d\'utilisateur est requis',
        emailRequired: 'L\'email est requis',
        emailExists: 'Cet email est déjÃ  utilisé par un autre utilisateur',
        passwordRequired: 'Le mot de passe est requis',
        fullNameRequired: 'Le nom complet est requis',
        userCreated: 'Utilisateur créé avec succès',
        userUpdated: 'Utilisateur mis Ã  jour avec succès',
  ...
  const handleDatabaseOperation = async () => {
    try {
      if (editingUser) {
        ...
        success(t.messages.userUpdated);
      } else {
        ...
        success(t.messages.userCreated);
      }
  ...
  const handleDeleteUser = async (userId: string) => {
    try {
      ...
      success(t.messages.userDeleted);
      await fetchUsers();
    } catch (err) {
                ...
                <tr key={user.id} className="hover:bg-nzoo-gray/5 transition-colors duration-200">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border font-montserrat ${getRoleColor(user.role)}`}>
                      ...
                      {user.role === 'clients' && <User className="w-3 h-3 mr-1" />}
                      {t.roles[user.role]}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
