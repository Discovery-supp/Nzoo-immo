...
// Charge un logo depuis le dossier public et le convertit en Data URL (avec fallback SVG)
const loadLogoDataUrl = async (): Promise<string> => {
  const toDataUrl = (blob: Blob): Promise<string> => new Promise(resolve => {
  ...
  // Priorité: header SVG demandé, puis PNG, puis SVG générique
  const headerSvg = await tryFetch('/logo-nzoo-immo-header.svg');
  if (headerSvg) return headerSvg;
  const png = await tryFetch('/logo_nzooimmo.png');
  if (png) return png;
  const svg = await tryFetch('/logo_nzooimmo.svg');
  if (svg) return svg;
  // Dernier fallback: data URI vide (évite image cassée)
  return 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"></svg>';
};

// Fonction pour générer le contenu HTML de la facture de réservation
const generateReservationInvoiceHTML = (data: ReservationData, logoSrc: string): string => {
  ...
  return `
    ...
    <body>
      <div class="invoice-container">
        <div class="header">
          <div class="logo">
            <img src="${logoSrc}" alt="N'zoo Immo Logo">
          </div>
          <h1>FACTURE</h1>
...
export const generateAndDownloadReservationInvoice = async (reservationData: ReservationData): Promise<void> => {
  try {
    ...
    const companyInfo = {
      ...
      logo: "/logo-nzoo-immo-header.svg"
    };

    // Charger le logo en Data URL pour garantir l'affichage hors-ligne
    const logoSrc = await loadLogoDataUrl();

    // Générer le HTML de la facture
    const htmlContent = generateReservationInvoiceHTML(reservationData, logoSrc);
...
const generateInvoiceHTML = (data: InvoiceData, logoSrc: string): string => {
  ...
  return `
    ...
    <body>
      <div class="invoice-container">
        <div class="header">
          <div class="logo">
            <img src="${logoSrc || companyInfo.logo || ''}" alt="N'zoo Immo Logo">
          </div>
          <h1>FACTURE</h1>
          <p>N'zoo Immo - Espaces de Travail</p>
        </div>
...
export const generateAndDownloadInvoice = async (reservation: Reservation): Promise<void> => {
  try {
    ...
    const companyInfo = {
      ...
      phone: "+243822201758",
      email: "contact@nzoo-immo.com",
      logo: "/logo-nzoo-immo-header.svg" // Chemin mis à jour vers le header SVG
    };
    ...
    // Charger le logo en Data URL pour affichage fiable après téléchargement
    const logoSrc = await loadLogoDataUrl();

    // Générer le HTML de la facture
    const htmlContent = generateInvoiceHTML(invoiceData, logoSrc);
