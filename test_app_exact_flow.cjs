...
// Fonction exacte de l'application (copie de reservationService.ts)
...
// Test principal - simulation exacte de l'application
async function testAppExactFlow() {
  console.log('🔍 TEST EXACT DU FLUX DE L\'APPLICATION');
  console.log('=======================================');
  
  // Données exactes comme dans l'application
  const formData = {
    fullName: 'Test Application Flow',
  ...
  try {
    ...
    // Appeler la fonction de réservation exactement comme l'application
...
testAppExactFlow().catch(console.error);
